			...
			<h1 class="page-title"><?php _e( 'Customer Ratings', 'claue' ); ?></h1>
				<?php if ( $vendor_feedback ) { 
					...
					foreach ( $vendor_feedback as $vf ) {
						...
						<p class="vendor-product-title"><?php _e( 'Product:', 'claue'); ?><a href="<?php echo $product_link; ?>" target="_blank"><?php echo $product_title; ?></a></p>
						<p class="rating-meta"><span><?php __( 'Posted on', 'claue'); ?> <?php echo $post_date; ?></span> <?php __( 'by', 'claue'); echo $customer_name; ?></p>
						<p class="vendor-product-comment"><?php echo $comment; ?></p>
						<hr />	
						</div>		
				...
				<h3 class="return-store"><a href="<?php echo $vendor_shop_url; ?>"><?php _e( '<span>&larr;</span> Return to store', 'claue' ); ?></a></h3>
			</div>
