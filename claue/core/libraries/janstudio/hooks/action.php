...
if ( ! function_exists( 'jas_claue_setup' ) ) {
	function jas_claue_setup() {
		 ...
		 * @since 1.0.0
		 */
		$GLOBALS['content_width'] = apply_filters( 'claue_content_width', 820 );
...
if ( ! function_exists( 'jas_claue_register_widgets' ) ) {
	function jas_claue_register_widgets() {
		...
		foreach ( $widgets as $widget ) {
			include JAS_CLAUE_PATH . '/core/libraries/janstudio/widgets/' . $widget . '.php';
		}
 ...
 * @since 1.1.3
 */
if ( ! function_exists( 'jas_claue_social_meta' ) && ! function_exists( 'wpseo_activate' ) ) {
	function jas_claue_social_meta() {
		$output = '';
		$image_src_array = wp_get_attachment_image_src( get_post_thumbnail_id(), 'full', true );
		
		$output .= '<meta itemprop="name" content="' . esc_attr( strip_tags( get_the_title() ) ) . '">';
		$output .= '<meta itemprop="description" content="' . esc_attr( strip_tags( get_the_excerpt() ) ) . '">';
		...
		$output .= '<meta name="twitter:description" content="' . esc_attr( strip_tags( get_the_excerpt() ) ) . '">';
	...
	add_action( 'wp_head', 'jas_claue_social_meta' );
}